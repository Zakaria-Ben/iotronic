...
_DEFAULT_RETURN_FIELDS = ('uuid', 'board_uuid', 'MAC_add', 'VIF_name', 'network', 'ip')
...
class Port(base.APIBase):
    ...
    def _convert_with_links(port, url, fields=None):
        ...
        port.links = [link.Link.make_link('self', url, 'ports',
                                            port_uuid),
...
class PublicPortsController(rest.RestController):
    """REST controller for Public Plugins."""

    invalid_sort_key_list = ['extra', 'location']

    def _get_ports_collection(self, marker, limit,
                                sort_key, sort_dir,
                                fields=None):
    ...
    def get_all(self, marker=None,
        ...
        policy.authorize('iot:port:get', cdict, cdict)
